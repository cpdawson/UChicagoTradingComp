...
class MyXchangeClient(xchange_client.XChangeClient):
    ...
    async def compute_largest_order(self):
        ...
        for symbol in symbols:
            ...
            print(self.order_trackers[symbol].__init__())
            if sorted_bids:
                large_order = sorted_bids[0]
                if large_order[1] > 100:
                    if self.order_trackers[symbol].__init__() is not None:
                        ...
                        if (large_order[0] - 1) not in orders:
                            print(f"LARGE BUY ORDER FOR {symbol}")
                            await self.place_and_track_order(symbol, 1, xchange_client.Side.BUY, large_order[0] + 1)
            ...
            if sorted_asks:
                ...
                if large_order[1] > 100:
                    if self.order_trackers[symbol].__init__() is not None:
                        ...
                        if (large_order[0] - 1) not in orders:
                            print(f"LARGE SELL ORDER FOR {symbol}")
                            await self.place_and_track_order(symbol, 1, xchange_client.Side.SELL, large_order[0] - 1)
                        else:
                            print("ORDER ALREADY OUTSTANDING")
                    else:
                        await self.place_and_track_order(symbol, 1, xchange_client.Side.SELL, large_order[0] - 1)


    async def bot_handle_book_update(self, symbol: str) -> None:
        await self.compute_best_bid_ask(symbol)
